...
class MysqliDb
{
     ...
     * @return mysqli_stmt
     */
    public function getpages($tableName,$where,$row='5')
    {
        $results=$this->query("SELECT COUNT(*) FROM $tableName WHERE $where");
        $page=ceil($results[0]['COUNT(*)']/$row);
        return $page;
    }
    public function getlists($tableName,$where,$row='5')
    {
        $page=(int)$_GET['page'];
        if($page<=0)$page=1;
        $limit=($page-1)*$row;
        $results=$this->query("SELECT * FROM $tableName WHERE $where LIMIT $limit,$row");
        return $results;
    }
    public function curPageURL()
    {
        ...
        $pageURL=str_replace("?page=$page&", '&', $pageURL);
        $pageURL=str_replace("?page=$page", '', $pageURL);
        #$pageURL=preg_replace("/&page=$page&/", '&', $pageURL);
        #$pageURL=preg_replace('/\?page=(.*)&/', '?', $pageURL);
        return $pageURL;
    }
    public function page($tableName,$where,$row='5'){
        $page=(int)$_GET['page'];
        $all=$this->getpages($tableName,$where,$row);    
        $s=$page-2;
        $s=($s>1)?$s:'1';
        $s=($s>$all)?$all:$s;
        $e=$page+2;
        ...
        $e=($page==2)?'5':$e;
        $e=($page==3)?'5':$e;
        $s=(($all-$page)==0)?($all-4):$s;
        $s=(($all-$page)==1)?($all-4):$s;
        $s=(($all-$page)==2)?($all-4):$s;
        }else{
        $e=$all;
        }
        $href=$this->curPageURL();
        $active[$page]='class="active"';
        $ss=$page-1;
        if($ss<1)$ss=1;
        $ee=$page+1;
        if($ee>=$all)$ee=$all;
        $p='<li><a href="'.$href.'&page='.$ss.'">&laquo;</a></li>';
        for($i=0;$i<5;$i++):
          $t=$s+$i;
          if($t<=$e):
          $p.='<li '.$active[$t].'><a href="'.$href.'&page='.$t.'">'.$t.'</a></li>';
          endif;
        endfor;

        $p.='<li><a href="'.$href.'&page='.$ee.'">&raquo;</a></li>';
        return $p;
    }
    /**
